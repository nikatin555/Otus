...
-- Можно вместо `REPLICATE_WILD_IGNORE_TABLE` использовать белый список, `Replicate_Wild_Do_Table` для исключения реплицирования новых таблиц, но в моём тесте, почему-то, с данной настройкой `REPLICATE_WILD_DO_TABLE = ('bet.bookmaker', 'bet.competition', 'bet.market', 'bet.odds', 'bet.outcome');` не синхронизировались все таблицы и даже новые. Решение я нашёл, описал в пунте "4)", раздела `Проблемы, с которыми столкнулся, во время выполнения домашнего задания` ,но переделывать не стал, воизбежании других проблем при тестировании.
