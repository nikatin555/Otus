...
# Вариант 1: решение для сбора и хранения логов с использованием Vagrant

`Vagrantfile`, который разворачивает инфраструктуру на Ubuntu 22.04:
...
└── ansible/
    ...
    ├── roles/
    │   ├── web/tasks/main.yml         # Роль для web-сервера
    │   ├── log/tasks/main.yml         # Роль для log-сервера
    │   └── elk/tasks/main.yml         # Роль для ELK
...
**Развертывание инфраструктуры**:
   ...
   vagrant up
   ```
   PS: на AlamaLinux 10 заметил, что с 1го раза не разворачиваются все VM или все приложения, п.э. приходится vagrant запускать повторно (на Windows таких проблем нет):
   ```bash
...
Для более сложной настройки можно добавить переменные в `group_vars/`, а также использовать теги для выборочного выполнения задач.