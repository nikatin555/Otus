...
Часть 1. <br>>
    1. Создать ВМ Ubuntu 24 с 2 ядрами и 4 Гб ОЗУ и SSD 10GB <br>
    2. Установить на него PostgreSQL 17 с дефолтными настройками <br>
    3. Создать БД для тестов: выполнить pgbench -i postgres <br>
    4. Запустить pgbench -c8 -P 6 -T 60 -U postgres postgres <br>
    5. Применить параметры настройки PostgreSQL из прикрепленного к материалам занятия файла <br>
    6. Протестировать заново <br>
    7. Что изменилось и почему? <br>
Часть 2. <br>
    8. Создать таблицу с текстовым полем и заполнить случайными или сгенерированными данным в размере 1млн строк <br>
    9. Посмотреть размер файла с таблицей <br>
    10. 5 раз обновить все строчки и добавить к каждой строчке любой символ <br>
    11. Посмотреть количество мертвых строчек в таблице и когда последний раз приходил автовакуум <br>
    12. Подождать некоторое время, проверяя, пришел ли автовакуум <br>
    13. 5 раз обновить все строчки и добавить к каждой строчке любой символ <br>
    14. Посмотреть размер файла с таблицей <br>
    15. Отключить Автовакуум на конкретной таблице <br>
    16. 10 раз обновить все строчки и добавить к каждой строчке любой символ <br>
    17. Посмотреть размер файла с таблицей <br>
    18. Объясните полученный результат <br>
    19. Не забудьте включить автовакуум <br>
    20. Задание со *:Написать анонимную процедуру, в которой в цикле 10 раз обновятся все строчки в искомой таблице.Не забыть вывести номер шага цикла. <br>
...
2. Заполняет их тестовыми данными:
   ...
   - Размер данных ~24MB (зависит от версии PostgreSQL)
...
Результаты (CPU на 13%, RAM <500MB) указывают на то, что pgbench не создает достаточной нагрузки для нашей системы. Разберём причины и решения:
...
Но в выводе следующей команды, критических проблем не выявлено, которые могли помешать тестированию:  watch -n 1 "cat /proc/interrupts | head -n 5": <br>
![alt text](image-10.png)

Диск (iostat) <br>
![alt text](image-9.png)

Даже оптимизация настроек для моего железа улучшения результата не дало:
...
1. PAE (Physical Address Extension)
    Позволяет 32-битным системам обращаться к более чем 4 ГБ оперативной памяти (теоретически до 64 ГБ). Для 64-битных систем PAE обычно не требуется, так как они изначально поддерживают большие объёмы памяти.
2. NX (No eXecute) Bit
...
Результат настройки гипервизора на лицо. Но и наш результат тестирования изменился, показывает, что наши настройки чуть менее эффективны в данном тесте, чем по default.
...
После изменений настроек VM,  tps вырос в несколько раз. Это показывает, на сколько важны настройки систем и важна работа **системного администратора**.
...
```
Размера файла с таблицей можно посмотреть:
```sql
...
Чтобы 5 раз обновить все строки в таблице `test_text`, добавляя к полю `content` символ на каждой итерации, можно использовать несколько методов. Вот несколько способов:
...
BEGIN
  FOR i IN 1..5 LOOP
    UPDATE test_text SET content = content || 'x';  -- добавляем символ 'x'
...
```sql
-- Итерация 1
UPDATE test_text SET content = content || 'a';

-- Итерация 2
UPDATE test_text SET content = content || 'b';

-- Итерация 3
UPDATE test_text SET content = content || 'c';

-- Итерация 4
UPDATE test_text SET content = content || 'd';

-- Итерация 25
UPDATE test_text SET content = content || 'e';
```
...
```bash
for i in {1..5}; do
  psql -U postgres -d your_db -c "UPDATE test_text SET content = content || '$i';"
done
```
...
DO
```
После 5 обновлений проверяю : <br>
